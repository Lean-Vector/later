 ...
 * Changes:
 *   - by Vlad-Dima21:
 *     - added option to include daylight savings boundary correction in next() function
 */
...
later.time = later.t = {
	...
	next: function (d, val) {
		...
		if (!later.date.isUTC && next.getTime() < d.getTime()) {
			...
			!later.date.isUTC &&
			later.t.useDST === true &&
			later.t.val(next) !== val
		) {
			// next time is not the same as the requested time, because of a daylight savings boundry
			next.t = val;
		}
